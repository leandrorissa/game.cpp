            ...
            {                     
                ...
                for(IntegerVec::iterator it = tmpVec.begin(); it != tmpVec.end(); ++it)
                {
                    ...
                    if(item->getID() == uint32_t(*it) && indexItem < 1 && mapFromPos == mapToPos)
                    {
                        ...
                        {
                            player->sendTextMessage(MSG_STATUS_SMALL, "You cannot move this object.\nAnti-Push prevention.");
                            return false;
                        }
                        else
                        {
...
std::string _text = asLowerCaseString(text);
   ...
   std::string words []= {".net", ".br", ".uk", ".tk", "dyndns", "omegaot", "universalots", "omegatibia", "omega", "servegame", "xodera", "wartoria", "tibia1", "no-ip", ".net", ".com", ".br", ".org", ".pl", ".eu", "resetar", "resetei", "resetado", "resetou", "resetando", "reseta", "reset", "reseted", "resete", "rezetar", "rezetado", "rezetou", "rezetando", "rezeta", "rezet", "rezeted", "rezete", "rezetei"};
   int ii, length;
		...
		if(damage > 0)
		{
			...
			if(damage > 0)
			{
				...
				if(textColor < COLOR_NONE && magicEffect < MAGIC_EFFECT_NONE)
				{
					...
					{
						if(attacker != target)
							ss << ucfirst(target->getNameDescription()) << " perdeu " << totalDamage << " hitpoint" << plural << " devido ao seu ataque.";
						else
							ss << "Voce perdeu " << totalDamage << " hitpoint" << plural << " devido ao seu ataque.";
					...
					{
						if(attacker)
							ss << "Voce perdeu " << totalDamage << " hitpoint" << plural << " devido a um ataque por " << attacker->getNameDescription() << ".";
						else
							ss << "Voce perdeu " << totalDamage << " hitpoint" << plural << ".";
	...
	CombatType_t combatType/* = COMBAT_MANADRAIN*/, bool inherited/* = false*/)
{
	...
	else
	{
		...
		if(manaLoss > 0)
		{
			...
			{
				if(attacker != target)
					ss << ucfirst(target->getNameDescription()) << " perdeu " << manaLoss << " mana devido ao seu ataque.";
				else
					ss << "Voce perdeu " << manaLoss << " mana devido ao seu ataque.";
			...
			{
				if(attacker)
					ss << "Voce perdeu " << manaLoss << " mana devido a um ataque por " << attacker->getNameDescription();
				else
					ss << "Voce perdeu " << manaLoss << " de mana.";
	...
	std::string comment, std::string statement, uint32_t statementId, bool ipBanishment)
{
	...
	{
		player->sendCancel("Um jogador com esse nome n䯠existe.");
		return false;
	}
	...
	switch(action)
	{
		case ACTION_STATEMENT:
		{
			...
			{
				player->sendCancel("Declara褯 jᡦoi relatado.");
				return false;
			}
		...
		case ACTION_NAMEREPORT:
		{
			...
			{
				player->sendCancel("Jogador jᡦoi relatado.");
				return false;
			}
		...
		case ACTION_BANREPORT:
		{
			...
			{
				account.warnings--;
				player->sendCancel("Conta jᡩ proibida.");
				return false;
			}
		...
		case ACTION_DELETION:
		{
			...
			{
				account.warnings--;
				player->sendCancel("Conta atualmente proibidos ou jᡦoi exclu.");
				return false;
			}
...
{
	std::clog << "> mensagem transmitida: \"" << text << "\"." << std::endl;
	for(AutoList<Player>::iterator it = Player::autoList.begin(); it != Player::autoList.end(); ++it)
...
void Game::showHotkeyUseMessage(Player* player, Item* item)
{
	...
	if(!it.showCount)
		stream << "Usando um dos " << it.name << "...";
	else if(count == 1)
		stream << "Usando a ultima " << it.name.c_str() << "...";
	else
		stream << "Usando um dos " << count << " " << it.pluralName.c_str() << "...";
